...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        train_log = self.actor.update(observations, actions, advantages, qvals=qvals)
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            q_values = np.hstack(self._discounted_return(rewards_list) for rewards_list in rewards_list)

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            q_values = np.hstack(self._discounted_cumsum(rewards_list) for rewards_list in rewards_list)
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            value_mean, value_std = np.mean(values_unnormalized), np.std(values_unnormalized)
            values = values_unnormalized - value_mean + q_mean
            values = values * (q_std / value_std)
    ...
    def _discounted_return(self, rewards):
        ...
        for t in range(len(rewards)):
            discounted_returns += rewards[t]*(self.gamma**t)

        list_of_discounted_returns = np.zeros(len(rewards))+discounted_returns 
    ...
    def _discounted_cumsum(self, rewards):
        ...
        rewards_to_go = np.zeros(len_of_rewards)
        rewards_to_go[len_of_rewards-1] = rewards[len_of_rewards-1]
        for i in reversed(range(len_of_rewards-1)):
            rewards_to_go[i] = rewards[i] + self.gamma*rewards_to_go[i+1]

        list_of_discounted_cumsums = rewards_to_go.copy()    
